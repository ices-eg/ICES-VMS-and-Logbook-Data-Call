...
#Specify data path
veDataPath <- "C:/Work/VMS/230405_ICES_data_call/table1Save.csv"

leDataPath <- "C:/Work/VMS/230405_ICES_data_call/table2Save.csv"
...
ve %>% 
  count(year, dcf4, gear, dcf6) %>% 
  ggplot(aes(year, n, colour = gear, group = dcf6)) +
  geom_point() +
  geom_line() +
  facet_wrap(~ dcf4, scales = "free_y") +
  labs(x = NULL, y = NULL,
...
d <- 
  ve %>% 
  filter(gear %in% top4) %>% 
  group_by(gear, year, lon, lat) %>% 
  summarise(fishing_days = sum(effort) / 24,
            .groups = "drop")
# do plots
for (gear in top4) {
  
  tmp <- d %>% filter(gear == gear)
  core  <- list(xlim = quantile(tmp$lon, c(0.025, 0.975)),
                ylim = quantile(tmp$lat, c(0.025, 0.975)))
  
  p <-
    map0 +
    geom_tile(data = d %>% filter(gear == gear),
              aes(lon, lat, fill = fishing_days)) +
    facet_wrap(~ year, ncol = 2) +
    coord_quickmap(xlim = core$xlim, ylim = core$ylim) +
    scale_fill_viridis_c(trans = "log", option = "B", guide = "legend",
...
VETargetAssemblage <- left_join(ve, TargetAssemblage, by = c("gear" = "Key"))%>%
  select(gear, VocabTargetAssemblage) %>%
  distinct()
...
LETargetAssemblage <- left_join(le, TargetAssemblage, by = c("gear" = "Key"))%>%
  select(gear, VocabTargetAssemblage) %>%
  distinct()
...
```{r}
le %>% count(year, dcf6, name = "logbook records") %>% 
  full_join(ve %>% count(year, dcf6, name = "vms records")) %>% 
